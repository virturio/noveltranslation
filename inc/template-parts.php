...
if (!function_exists('novo_get_breadcrumb_items')) {
    /**
     ...
     */
    function novo_get_breadcrumb_items($post_id)
    {
...
if (!function_exists('novo_get_breadcrumb_icon')) {
    /**
     ...
     */
    function novo_get_breadcrumb_icon($type)
    {
        if ($type === 'separator') {
...
if (!function_exists('novo_construct_breadcrumb')) {
    /**
     ...
     * @return void
     */
    function novo_construct_breadcrumb($post_id)
    {
        $items = novo_get_breadcrumb_items($post_id);
        ?>
                ...
                <?php foreach ($items as $index => $item): ?>
                    <li <?php echo isset($item['current']) ? 'aria-current="page"' : ''; ?>>
                        <div class="flex items-center">
                            <?php if ($index > 0): ?>
                                <?php echo novo_get_breadcrumb_icon('separator'); ?>
                            <?php endif; ?>

                            <?php if (isset($item['icon'])): ?>
                                <?php echo novo_get_breadcrumb_icon('home'); ?>
                            <?php endif; ?>
...
if (!function_exists('novo_get_dropdown_icons')) {
    /**
     ...
     */
    function novo_get_dropdown_icons($context)
    {
        $leading_icon = null;
...
if (!function_exists('novo_dropdown_handler')) {
    ...
    {
        $icons = novo_get_dropdown_icons($context);
        ?>
        <button <?php novo_do_attr('filter-button'); ?>>
            <?php novo_button_content(
    ...
    function novo_construct_searchbar()
    {
        ...
        <div class="flex-1 mx-2 h-[44px]">
            <div class="search-container h-full relative transition-all duration-300 ease">
                ...
                <?php echo novo_get_search_loading_indicator(); ?>
...
if (!function_exists('novo_get_search_icon')) {
    /**
     * Get search icon HTML
     * 
     * @return string Search icon HTML
     */
    function novo_get_search_icon()
    {
        return '<svg class="h-6 text-novo-text-gray search-icon transition-all duration-300 ease group-hover:text-white group-hover:scale-110 group-focus-within:text-white group-focus-within:scale-110"
...
if (!function_exists('novo_get_search_loading_indicator')) {
    /**
     * Get search loading indicator HTML
     * 
     * @return string Loading indicator HTML
     */
    function novo_get_search_loading_indicator()
    {
        return '<div id="search-loading" class="hidden absolute top-full left-0 right-0 mt-2 bg-novo-search rounded-lg p-4 border border-novo-border">
